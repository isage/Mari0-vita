...
--	controls[i]["use"] = {"e"}
	...
	controls[i]["right"] = {"joy", 1, "but", 10}
	controls[i]["left"] = {"joy", 1, "but", 8}
	controls[i]["down"] = {"joy", 1, "but", 7}
	controls[i]["up"] = {"joy", 1, "but", 9}
	controls[i]["run"] = {"joy", 1, "but", 2}
	controls[i]["jump"] = {"joy", 1, "but", 3}
	controls[i]["aimx"] = {"joy", 1, "axe", 3, "pos"}
	controls[i]["aimy"] = {"joy", 1, "axe", 4, "pos"}
	controls[i]["portal1"] = {"joy", 1, "but", 5}
	controls[i]["portal2"] = {"joy", 1, "but", 6}
	controls[i]["reload"] = {"joy", 1, "but", 1}
	controls[i]["use"] = {"joy", 1, "but", 4}
	controls[i]["esc"] = {"joy", 1, "but", 12}
...
--	love.window.setMode(width*16*scale, 224*scale, {fullscreen=fullscreen, vsync=vsync}) --27x14 blocks (15 blocks actual height)
	...
	gamewidth = 960 --love.graphics.getWidth()
	gameheight = 544 --love.graphics.getHeight()
